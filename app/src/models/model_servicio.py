...
class Model_servicio():
    ...
    def insertar(cls, db, servicio):
        try:
            ...
            cursor.callproc('InsertarServicio', (servicio.nombre, servicio.precio, servicio.duracion))
    ...
    def mostrar(cls, db):
        try:
            ...
            servicios = []
            for row in rows:
                nombre, precio, duracion, codServicio = row
    ...
    def mostrarU(cls, db, codServicio):
        try:
            ...
            if row:
                nombre, precio, duracion, codServicio = row
                return Servicio(nombre, precio, duracion, codServicio=codServicio)
            else:
                return None
        except Exception as ex:
            raise Exception(ex)
    ...
    def modificar(cls, db, servicio):
        try:
            cursor = db.connection.cursor()
            cursor.callproc('ActualizarServicio', (servicio.codServicio, servicio.nombre, servicio.precio, servicio.duracion))
            db.connection.commit()
        except Exception as ex:
            raise Exception(ex)
